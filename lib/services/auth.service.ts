...
export const authService = {
  ...
  // BUG #7 FIX: Removed authService.refresh().
  // This method used the `api` instance (with interceptors) to call /auth/refresh.
  // That would trigger the 401 interceptor recursively — infinite loop.
  // The correct way to refresh is via refreshAccessToken() from lib/api.ts,
  // which uses bare axios (no interceptors). Nothing in the codebase called
  // authService.refresh() — it was dead code that could mislead future developers.
