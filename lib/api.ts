...
// BUG #3 FIX: Removed dead failedQueue / processQueue implementation.
// The previous code declared failedQueue and processQueue but never pushed to
// failedQueue — so processQueue always iterated over an empty array.
// Concurrent 401 requests are correctly handled by sharing refreshPromise
// directly (all concurrent callers await the same promise). The queue
// abstraction added complexity without any functional benefit.
//
// refreshPromise is reset via .finally() on the promise chain itself, which
// is simpler and guarantees the reset happens regardless of success or failure.
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (error.response?.status === 401 && !original._retry) {
      ...
      // CQ-017 FIX: Uses bare axios.post() directly (NOT the api instance).
      ...
      //
      // BUG #3 FIX: refreshPromise is now reset via .finally() on the chain itself
      // instead of a finally{} block around await. This ensures the reset happens
      // exactly once regardless of how many concurrent callers await the promise,
      // and removes the need for the dead failedQueue safety-net code.
      if (!refreshPromise) {
        refreshPromise = axios.post<ApiResponse<{ accessToken: string }>>(
          ...
          // BUG #9 FIX: Removed redundant .catch() that only re-threw the error.
          // Error propagates naturally to the try/catch below.
          .finally(() => {
            // Always reset so the next 401 triggers a fresh refresh attempt.
            refreshPromise = null
          })
      }
      ...
      try {
        ...
        // Refresh failed — session is truly expired. Log out and redirect.
...
// BUG #1 FIX: extractData() was defined but never used in any query hook.
// All hooks were using the unsafe `res.data.data` pattern which silently
// returns null when the backend responds with { success: false, data: null }.
// React Query treats a resolved null as success (isError=false), so the
// UI shows the error state ("Gagal Memuat Profil") with no automatic recovery
// and no retry. extractData() throws on failure, which lets React Query set
// isError=true and trigger its retry/refetch cycle correctly.
//
// All authenticated query hooks now import and use extractData() instead of
// the raw `.then((res) => res.data.data)` pattern.
