...
export function useTransactions(params?: OrderListParams): UseQueryResult<TransactionListResult | null> {
  // AUTH-010 FIX: Added isAuthenticated guard
...
export function useTransactionDetail(id: string) {
  // AUTH-011 FIX: Added isAuthenticated check alongside id check
  const isAuthenticated = useAuthStore((s) => s.isAuthenticated)
  return useQuery({
    queryKey: transactionKeys.detail(id),
    queryFn: () => transactionService.getDetail(id).then((res) => res.data.data),
...
export function useOrderSummary() {
  ...
  return useQuery({
    ...
    queryFn: () => transactionService.getSummary().then((res) => res.data.data),
    enabled: isAuthenticated,
    staleTime: 60 * 1000,
  })
}
...
export function useOrderHistory(id: string) {
  // AUTH-012 FIX: Added isAuthenticated check
  const isAuthenticated = useAuthStore((s) => s.isAuthenticated)
  return useQuery({
    queryKey: transactionKeys.history(id),
    queryFn: () => transactionService.getHistory(id).then((res) => res.data.data),
    enabled: !!id && isAuthenticated,
  })
}

export function useOrderExtensions(id: string) {
  // AUTH-013 FIX: Added isAuthenticated check
  const isAuthenticated = useAuthStore((s) => s.isAuthenticated)
  return useQuery({
    queryKey: transactionKeys.extensions(id),
    queryFn: () => transactionService.getExtensions(id).then((res) => res.data.data),
    enabled: !!id && isAuthenticated,
  })
}
