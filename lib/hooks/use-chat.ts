...
export function useChatRooms(params?: PaginationParams) {
  // AUTH-016 FIX: Added isAuthenticated guard
  const isAuthenticated = useAuthStore((s) => s.isAuthenticated)
  return useQuery({
    queryKey: ['chat', 'rooms', params],
    queryFn: () => chatService.getRooms(params).then((res) => res.data.data),
...
export function useChatRoomMessages(roomId: string, params?: { cursor?: string; limit?: number }) {
  // AUTH-017 FIX: Added isAuthenticated check
  const isAuthenticated = useAuthStore((s) => s.isAuthenticated)
  return useQuery({
    queryKey: ['chat', 'room', roomId, 'messages', params],
    queryFn: () => chatService.getRoomMessages(roomId, params).then((res) => res.data.data),
...
export function useChatRoomAttachments(roomId: string, params?: PaginationParams) {
  // AUTH-018 FIX: Added isAuthenticated guard — was missing unlike other chat hooks
  const isAuthenticated = useAuthStore((s) => s.isAuthenticated)
  return useQuery({
    queryKey: ['chat', 'room', roomId, 'attachments', params],
    queryFn: () => chatService.getRoomAttachments(roomId, params).then((res) => res.data.data),
    enabled: !!roomId && isAuthenticated,
  })
}
...
export function useChatMessages(orderId: string, params?: { cursor?: string; limit?: number }) {
  // AUTH-019 FIX: Added isAuthenticated guard — was missing unlike useChatRoomMessages
  const isAuthenticated = useAuthStore((s) => s.isAuthenticated)
  return useQuery({
    queryKey: ['chat', orderId, params],
    queryFn: () =>
      chatService.getMessages(orderId, params).then((res) => res.data.data),
