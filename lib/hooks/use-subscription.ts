...
export function useSubscriptionStatus() {
  // AUTH-009 FIX: Subscription is paid data — added isAuthenticated guard
  const isAuthenticated = useAuthStore((s) => s.isAuthenticated)
  return useQuery({
    queryKey: ['subscription', 'status'],
    queryFn: () => subscriptionService.getStatus().then((res) => res.data.data),
...
export function useSubscriptionHistory(params?: PaginationParams) {
  ...
  return useQuery({
    ...
    queryFn: () => subscriptionService.getHistory(params).then((res) => res.data.data),
    enabled: isAuthenticated,
  })
}
